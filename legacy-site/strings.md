...
<br />
*In these examples, `my_variable` has the floating point value 4.29, `my_string` is `"hello"`, and `other_variable` has the integer value 8. In each case I give the output or effect in a comment*
<pre>
...
print(f"Result is {my_variable + 2.1}")                     # prints "Result is 6.39"  (you can do calculations inside the braces!)
...
print(f"Result to four d.ps is {my_variable:.4f}")          # prints "Result to four d.ps is 4.2900" (and again)
print(f"Result is {other_variable:02}")                     # prints "Result is 08" (02 means use two columns, padding with leading zeros)
print(f"Result is {other_variable:3}")                      # prints "Result is   8" (3 means use three columns, padding with spaces)
