...
To build a matplotlib chart, you use (a) a ‘graph generation function’ (my term, not matplotlibs) to create a ‘base’ graph, (b) optionally call ‘graph decoration functions’ (again, my term) to decorate the graph, e.g., with labels and legends, then (c) a final call to the *show* function to show the completed graph. For example, the graph below is made by:
